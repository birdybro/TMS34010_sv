...
// tb_pixblt_w2.sv
//
// PIXBLT XY window MISS detection, CONTROL.W=2 (Task 0108). Mirrors the FILL
// W=2 path (Task 0107) for the block-transfer engine (1988 UG §7.10.2): the
// destination array is transferred only if it lies entirely within the window
// (V=0); otherwise nothing is drawn, V=1, and INTPEND.WV is set.
//
// Geometry (tb_pixblt_xy): PSIZE=8, CONVSP=CONVDP=0x1B, OFFSET=0x800, DYDX =
// DX=2,DY=1, source word128 = 0x2211.
//   #1 DADDR=(0x20,0), window (0x20,0)..(0x21,0) CONTAINS array → transferred
//      (word144 = 0x2211), V=0.
//   #2 DADDR=(0x20,2), window (0x20,2)..(0x20,2) excludes X=0x21 → MISS: not
//      drawn (word146 stays 0), V=1, INTPEND.WV set.
// -----------------------------------------------------------------------------
...
module tb_pixblt_w2;
  ...
  localparam logic [15:0] CTRL_W2   = 16'(2'd2 << CTRL_W_LO);
  ...
  initial begin : main
    ...
    u_mem.mem[128] = 16'h2211;  // source pixels
    ...
    p = place_movi_il  (p, 4'd0, {16'h0, CTRL_W2});
    p = place_store_abs(p, 4'd0, A_CONTROL);             // CONTROL.W=2
    ...
    // #1: dest array inside window → transferred, V=0.
    p = place_movi_il_b(p, 4'd0, 32'h0000_0000);         // SADDR (0,0)
    p = place_movi_il_b(p, 4'd2, 32'h0000_0020);         // DADDR (0x20,0)
    p = place_movi_il_b(p, 4'd5, 32'h0000_0020);         // WSTART (0x20,0)
    p = place_movi_il_b(p, 4'd6, 32'h0000_0021);         // WEND   (0x21,0)
    p = place_word(p, 16'h0F60);                         // PIXBLT XY,XY
    p = place_word(p, getst_enc(4'd8));                  // A8 <- ST (V=0)

    // #2: dest crosses X=0x21 > WEND.X=0x20 → MISS, not drawn, V=1, WVP.
    p = place_movi_il_b(p, 4'd0, 32'h0000_0000);         // SADDR (0,0)
    p = place_movi_il_b(p, 4'd2, 32'h0002_0020);         // DADDR (0x20,2)
    p = place_movi_il_b(p, 4'd5, 32'h0002_0020);         // WSTART (0x20,2)
    p = place_movi_il_b(p, 4'd6, 32'h0002_0020);         // WEND   (0x20,2)
    p = place_word(p, 16'h0F60);                         // PIXBLT XY,XY
    p = place_word(p, getst_enc(4'd9));                  // A9 <- ST (V=1)
    ...
    check_word("PIXBLT#1 word144 = 0x2211 (inside, drawn)", 144, 16'h2211);
    check_word("PIXBLT#2 word146 = 0 (miss, not drawn)", 146, 16'h0000);
    if (u_core.u_regfile.a_regs[8][ST_V_BIT] !== 1'b0) begin
      $display("TEST_RESULT: FAIL: PIXBLT#1 V expected 0, A8=%08h", u_core.u_regfile.a_regs[8]);
      failures++;
    end
    if (u_core.u_regfile.a_regs[9][ST_V_BIT] !== 1'b1) begin
      $display("TEST_RESULT: FAIL: PIXBLT#2 V expected 1, A9=%08h", u_core.u_regfile.a_regs[9]);
      failures++;
    end
    if (u_core.u_io_regs.io_reg[IO_IDX_INTPEND][INT_WV_BIT] !== 1'b1) begin
      $display("TEST_RESULT: FAIL: INTPEND.WV not set after window miss: %04h",
    ...
    if (failures == 0)
      $display("TEST_RESULT: PASS (PIXBLT XY W=2: inside→drawn/V=0, miss→not-drawn/V=1/INTPEND.WV)");
    else
  ...
  initial begin : watchdog
    #4_000_000;
    $display("TEST_RESULT: FAIL: tb_pixblt_w2 hard timeout");
    $fatal(1);
  end
endmodule : tb_pixblt_w2
