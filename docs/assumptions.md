...
- **Field-machinery progress (Task 0078)**: the OFFSET (0xB000/0xB400) and ABSOLUTE (0x0580/0x05A0) MOVE forms are now field-size aware too — same `mv_fs`/`mv_load_data` machinery (no pointer step). tb_move_offset / tb_move_abs now SETF FS0=0 up front; new tb_move_offabs_field covers FS=8/16 and an FS=12 straddling absolute field. **Only the M2M (indirect↔indirect) MOVE forms remain FS=32** — those need FS carried through both the read and write steps of the 2-step CORE_MEMORY sequence (and the dual ±FS pointer step). MOVB (byte, FS fixed 8) falls out of the completed field machinery.
