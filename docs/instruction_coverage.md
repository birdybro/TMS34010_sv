...
| JRcc short | `1100 cccc dddd dddd` | SPVU001A A-14, Table 12-8 | partial: UC/EQ/NE + unsigned compares (A0017) | tb_jruc_short, tb_jrcc_short, tb_jrcc_unsigned | none | none | TBD | Conditional/unconditional 8-bit-signed-disp relative jump. Verified cc values: 0000 UC, 0001 LO (C=1), 0010 LS (C\|Z), 0011 HI (~C&~Z), 0100 EQ (Z=1), 0111 NE (Z=0), 1001 HS (!C). Signed-compare codes (LT/LE/GT/GE) still deferred. Other cc values trap as ILLEGAL. Target = PC_post_fetch + sign_extend(disp) × 16. disp ∈ {0x00, 0x80} reserved. |
