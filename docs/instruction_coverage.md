...
| RETS [N] | `0000 1001 011N NNNN` (= `0x0960 \| N`) | SPVU001A page 12-231 + summary table | implemented | tb_rets | **none** (Unaffected) | **read** (32-bit; pops PC) | TBD | `PC <- mem[SP]; SP <- SP + 32 + 16*N`. N at instr[4:0] (0..31) — optional argument-pop count after return. The popped PC value flows directly into the PC-load mux (no bottom-nibble mask, since pushed PCs are already word-aligned). |
