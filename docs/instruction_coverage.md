...
| CALL Rs | `0000 1001 001R DDDD` (= `0x0920 \| (R<<4) \| Rs`) | SPVU001A page 12-47 + summary table | implemented | tb_call_rs | **none** (Unaffected) | **write** (32-bit; pushes PC') | TBD | `SP -= 32; mem[SP] <- PC'; PC <- Rs` (with bottom 4 bits of PC forced to 0). PC' = address of the instruction following CALL. First subroutine-call instruction. Reuses the memory-write infrastructure from PUSHST plus the PC-load mux pattern from JUMP Rs. |
