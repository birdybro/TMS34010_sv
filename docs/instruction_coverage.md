...
| TRAP N | `0000 1001 000N NNNN` (= `0x0900 \| N`) | SPVU001A page 12-252 + summary table | implemented (N≥1; TRAP 0 deferred) | tb_trap | **N=0, C=0, Z=0, V=0** (ST forced to `0x00000010`) | **write** ×2 (push PC', push ST) + **read** ×1 (vector fetch) | TBD | Software-interrupt sequence: `SP -= 32; mem[SP] <- PC'; SP -= 32; mem[SP] <- ST; ST <- 0x00000010; PC <- mem[0xFFFFFFE0 - N*32]`. First three-transaction instruction: `mem_op_step` cycles 0→1→2 with per-step direction (write/write/read) and per-step address (SP-32 / SP-64 / vector). The vector address is at the very top of bit-address space; the sim memory model masks the address to its internal index range so the test can pre-place the vector at the aliased slot. ALU computes `SP - 64`. Post-TRAP ST is the spec-fixed `0x10` (IE=0, FS0=16, FS1=0, flags=0). TRAP 0 (the special non-pushing case) is not yet handled — see [docs/assumptions.md](assumptions.md). |
